...
impl Signature<'_> {
    ...
    /// Returns the input tensor associated with a signature binding index.
    pub fn input_tensor(&self, input_index: LiteRtParamIndex) -> Result<Tensor<'_>, Error> {
        let mut raw_tensor_ptr: LiteRtTensor = std::ptr::null_mut();
        call_check_status!(
            // SAFETY: self.raw_signature is valid and the C API validates input_index.
            unsafe {
            ...
            ErrorCause::GetSignatureInputTensor
        );
        Ok(Tensor {
    ...
    /// Returns the output tensor associated with a signature binding index.
    pub fn output_tensor(&self, output_index: LiteRtParamIndex) -> Result<Tensor<'_>, Error> {
        let mut raw_tensor_ptr: LiteRtTensor = std::ptr::null_mut();
        call_check_status!(
            // SAFETY: self.raw_signature is valid and the C API validates output_index.
            unsafe {
            ...
            ErrorCause::GetSignatureOutputTensor
        );
        Ok(Tensor {
            raw_tensor: raw_tensor_ptr,
            _phantom: PhantomData {},
